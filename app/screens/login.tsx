    ...
    try {
      if (isLogin) {
        ...
        if (res.ok && res.token) {
          ...
          Alert.alert("Erro no Cadastro", res.detail || "Erro desconhecido");
        }
      }
    } catch(e) {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.title}>{isLogin ? "Acesso Restrito" : "Novo Pesquisador"}</Text>
        <Text style={styles.subtitle}>
          {isLogin ? "Entre para contribuir com os datasets do Li-Vision Edge." : "Crie uma conta para catalogar suas doações de dados."}
        </Text>
        ...
        <TouchableOpacity style={styles.mainBtn} onPress={handleSubmit} disabled={loading}>
          {loading ? <ActivityIndicator color="#0c0f16"/> : (
            <Text style={styles.mainBtnText}>{isLogin ? "Entrar" : "Criar Conta"}</Text>
          )}
        </TouchableOpacity>

        <TouchableOpacity onPress={() => setIsLogin(!isLogin)} style={styles.toggleBtn}>
          <Text style={styles.toggleText}>
            {isLogin ? "Ainda não contribuiu? Crie sua conta!" : "Já possuo conta (Fazer Login)"}
          </Text>
        </TouchableOpacity>
      </View>
